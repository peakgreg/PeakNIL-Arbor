...
<body>
<main class="content ps-2 pe-2">
<?php
// Enhanced nav inclusion with fallback paths
error_log('Starting auth nav inclusion with fallbacks...');

// Define possible locations for the auth nav file
$nav_locations = [
    // Standard path
    ...
    '/var/www/html/modules/common/views/auth/view.nav.php',
    ...
    // Try backup nav file
    MODULES_PATH . '/common/views/auth/view.nav.bak.php',
    '/var/www/modules/common/views/auth/view.nav.bak.php'
];

// Try each location until we find one that exists
$nav_found = false;
foreach ($nav_locations as $nav_file) {
    error_log('Trying auth nav file: ' . $nav_file);
    
    if (file_exists($nav_file)) {
        error_log('SUCCESS: Auth nav file found at: ' . $nav_file);
...
if (!$nav_found) {
    error_log('WARNING: Auth nav file not found in any location');
    echo '<div style="background:#f8f8f8;padding:15px;margin-bottom:20px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <div>
                    ...
                    <a href="/settings" style="margin-right:15px;text-decoration:none;color:#333;">Settings</a>
